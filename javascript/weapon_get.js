const hasOwnProperty = require('has-own-property-x');
const util = require('util');


/*var weaponGet = (obj) => {
    if (obj){
        var atk = obj.atk
        var cost = obj.cost
        var hp = obj.hp
        //When using util for string formatting, you must bracket each variable seperated with commas
        var result = util.format('Weapon Stats: atk: %s, cost: %s, hp: %s', (atk),(cost),(hp));
        console.log(result)}
    else for (index in obj)
      weaponGet(obj[index])
    
    return 'Weapon stats returned'
    
  };*/

  var Sword = {
    'Butter knife': {atk: 1, cost: 0, hp: 1},
    'Long Sword': { atk: 3, cost: 50, hp: 3},
    'Great Sword': {atk: 7, cost: 100, hp: 7},
    'Katana': {atk: 20, cost: 200, hp: 20}
};

var Spear = {
    'Stick': {atk: 1, cost: 0, hp: 1},
    'Lance': { atk: 4, cost: 50, hp: 2},
    'Javelin': {atk: 10, cost: 100, hp: 4},
    'Glaive': {atk: 30, cost: 200, hp: 10}
};

var Axe = {
    'Shovel': {atk: 2, cost: 0, hp: 0},
    'Tomahawk': { atk: 6, cost: 50, hp: 1},
    'Battle Axe': {atk: 15, cost: 100, hp: 2},
    'Great Hammer': {atk: 50, cost: 200, hp: 5}
};

module.exports = {weaponGet};